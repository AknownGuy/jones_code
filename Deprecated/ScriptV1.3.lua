...
-- Theme colors
local COL_BG      = Color3.fromRGB(10, 10, 16)
local COL_PANEL   = Color3.fromRGB(18, 18, 28)
local COL_BORDER  = Color3.fromRGB(50, 50, 80)
local COL_ACCENT  = Color3.fromRGB(80, 120, 255)    -- blue  → generator
local COL_ACCENT2 = Color3.fromRGB(50, 185, 110)    -- green → breakers
local COL_ACCENT3 = Color3.fromRGB(210, 150, 40)    -- amber → player
local COL_ACCENT4 = Color3.fromRGB(200, 65, 65)     -- red   → danger / close
local COL_TEXT    = Color3.fromRGB(205, 205, 215)
local COL_SUB     = Color3.fromRGB(95, 95, 115)

-- Main frame  (330 × 460 — wider & taller than before)
...
-- Hint line  (shortened to fit cleanly in the wider frame)
...
local function makeButton(text, posY, height, bgCol, customSize, xOffset)
    ...
    btn.Size             = customSize or UDim2.new(1, -20, 0, height or 34)
...
-- ══════════════════════════════════════════
--   SECTION: BREAKERS   starts at y = 62
-- ══════════════════════════════════════════
...
-- Two side-by-side buttons (scan+teleport | activate-only)
local fireBtn = makeButton(
    "SCAN + ACTIVATE  [V]", 106, 34,
    Color3.fromRGB(24, 72, 50),
    UDim2.new(0.62, -14, 0, 34), 10
)
fireBtn.TextSize = 11

local activateBtn = makeButton("ACTIVATE", 106, 34, Color3.fromRGB(20, 45, 38))
activateBtn.Position = UDim2.new(0.62, 0, 0, 106)
activateBtn.Size     = UDim2.new(0.38, -10, 0, 34)
...
-- ══════════════════════════════════════════
--   SECTION: GENERATOR   starts at y = 152
-- ══════════════════════════════════════════
makeSection("GENERATOR", 152, COL_ACCENT)

local genStatus = makeStatus(170, COL_ACCENT)
genStatus.Text = "Status: ready   (A → Z)   bind: C"

local genProg = makeProgress(188, COL_ACCENT)

local genBtn = makeButton("SOLVE GENERATOR  [C]", 196, 34, Color3.fromRGB(28, 42, 105))
...
local function runGenerator()
    ...
    task.spawn(function()
        ...
        genStatus.Text       = "Done — all letters sent"
...
-- ══════════════════════════════════════════
--   SECTION: PLAYER   starts at y = 242
-- ══════════════════════════════════════════
makeSection("PLAYER", 242, COL_ACCENT3)

-- ── God Mode ──
local godToggle = makeToggleRow("God mode  (auto-heal to max HP)", 260, Color3.fromRGB(28, 95, 55), Color3.fromRGB(55, 18, 18))
...
dmgRow.Position         = UDim2.new(0, 10, 0, 296)
...
local dmgFireStatus = makeStatus(330, COL_ACCENT3)
dmgFireStatus.Text = "Status: ready"

local dmgBtn = makeButton("HEAL PLAYER", 346, 34, Color3.fromRGB(30, 80, 45))
...
-- ── Night Vision ──
local nvToggle = makeToggleRow("Night vision  (fires every 10s)", 388, Color3.fromRGB(28, 55, 130), Color3.fromRGB(55, 18, 18))
nvToggle.BackgroundColor3 = Color3.fromRGB(55, 18, 18)

local nvStatus = makeStatus(422, Color3.fromRGB(140, 175, 255))
...
nvToggle.MouseButton1Click:Connect(function()
    ...
    if nightVisionEnabled then
        ...
        nvStatus.Text             = "Active — fires every 10s"
...
local function onBreakerDone(success, total)
    ...
    task.wait(2)    -- wait 2s before resetting UI
...
-- Fire WITH teleport: go to (0, 5.5, 6) → wait 0.5s → activate → wait 2s
local function startFire()
    ...
    task.spawn(function()
        breakerStatus.Text       = "Teleporting to breakers..."
...
print("[Menu] v1.3 loaded — G = menu | V = breakers | C = generator")