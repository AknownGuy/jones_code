-- Menu V1.5 | G=menu | V=breakers | F=generator | X=cursor | H=autofloor
...
local function fireElevatorTrigger()
    ...
    -- fire both TouchInterests that live inside the trigger part
...
-- ──────────────── CURSOR LOCK ────────────────
-- Uses a RenderStepped loop to fight the game's first-person lock
...
local function setCursorLock(locked)
    ...
    if not locked then
        -- override the game's per-frame lock
...
local function runBacteriumAvoid(onStatus)
    ...
    if #prompts == 0 then
        if onStatus then onStatus("No Bacterium on this floor") end
        return
    end
    ...
    for _, prompt in ipairs(prompts) do
        ...
        if part and part:IsA("BasePart") then
            ...
            if onStatus then onStatus("Triggered " .. done .. "/" .. #prompts .. " Bacterium") end
            task.wait(0.08)
            hrp.CFrame = origin
            task.wait(0.12)
        end
    ...
    if onStatus then onStatus("Bacterium done: " .. done .. " triggered") end
end
...
-- (needed by autoFloor which is defined before startFire)
local startFire
...
local function startAutoFloor(onStatus)
    ...
    autoFloorThread = task.spawn(function()
        while autoFloorEnabled do
            ...
            if floorsFolder and #floorsFolder:GetChildren() > 0 then
                if onStatus then onStatus("Floor detected — waiting 3s...") end
                task.wait(3)
                if not autoFloorEnabled then break end
                if onStatus then onStatus("Running breakers...") end
                startFire()
                task.wait(0.8)                 -- small buffer before polling
                while busy do task.wait(0.3) end  -- wait for completion
                if onStatus then onStatus("Done — waiting 2s...") end
                task.wait(2)
            else
                if onStatus then onStatus("Waiting for floor...") end
                task.wait(1)
...
local function makeTabBtn(text, xScale, xOff, wScale, wOff)
    ...
    b.Size               = UDim2.new(wScale, wOff, 1, -4)
...
local tabEntities = makeTabBtn("ENTITIES", 0.667, 1,  0.333, -3)
...
local entitiesPage = makePage(false)

local function setTab(which)
    mainPage.Visible     = (which == "main")
    bindsPage.Visible    = (which == "binds")
    entitiesPage.Visible = (which == "entities")
    for id, btn in pairs({ main = tabMain, binds = tabBinds, entities = tabEntities }) do
...
tabEntities.MouseButton1Click:Connect(function() setTab("entities") end)
setTab("main")

-- ════════════════════════════════════════════════
--  UI HELPERS (all take explicit parent)
-- ════════════════════════════════════════════════
...
--  ENTITIES PAGE
-- ════════════════════════════════════════════════
uiSection(entitiesPage, "BACTERIUM AVOID", 10, CA4)

local bacteriumToggle, batActive, batInactive = uiToggleRow(
    entitiesPage, "Avoid Bacterium (auto-trigger)", 28,
    Color3.fromRGB(130,30,30), Color3.fromRGB(55,18,18)
)
bacteriumToggle.BackgroundColor3 = Color3.fromRGB(55,18,18)

local batStatus = uiStatus(entitiesPage, 62, CA4)
batStatus.Text = "Bacterium: off"

local batInfo = Instance.new("TextLabel")
batInfo.Text = "Teleports to each Bacterium on the current floor and fires its proximity prompt, then returns. Polls every 5s. Only active when a floor is loaded."
...
bacteriumToggle.MouseButton1Click:Connect(function()
    ...
    if bacteriumEnabled then
        batStatus.Text = "Bacterium: active"
...
-- Auto floor toggle (shared logic, called by button and keybind)
...
-- ════════════════════════════════════════════════
--  ACTIVATE + TP  (assigned to forward-declared startFire)
--
--  Sequence:
--    1. Teleport to 0, 5, 5
--    2. Run scanAndFire
--    3. Wait 0.2s
--    4. Fire both elevator TouchInterests
--    5. Wait 0.2s
--    6. Teleport to 20, 5, 5
-- ════════════════════════════════════════════════
startFire = function()
    if busy then return end
    ...
    task.spawn(function()
        ...
        -- Step 1
        breakerStatus.Text       = "Teleporting to 0,5,5..."
        ...
        scanAndFire(
            ...
            function(success, total)
                ...
                -- Step 4 — fire both elevator TouchInterests
                breakerStatus.Text       = "Triggering elevator..."
                ...
                -- Step 6
                breakerStatus.Text       = "Returning to 20,5,5..."
                breakerStatus.TextColor3 = Color3.fromRGB(120,160,255)
                teleportTo(20, 5, 5)

                -- Fallback: if FloorReached hasn't changed, try firing Floors remote
                task.wait(0.3)
...
-- ACTIVATE only (no teleport, no elevator fire)
...
local function unload()
    scriptActive = false           -- kills all keybind handlers
    stopNightVision()
    stopGodMode()
    stopBacterium()
    stopAutoFloor()
    setCursorLock(false)           -- always restore cursor
    if cursorConn then cursorConn:Disconnect(); cursorConn = nil end
    task.wait(0.05)
    if screenGui and screenGui.Parent then
        screenGui:Destroy()
    end
    print("[Menu] V1.5 unloaded — all loops stopped, keybinds dead")
end

unloadBtn.MouseButton1Click:Connect(unload)

print("[Menu] V1.5 loaded — " ..
    getKeyName(keybinds.menu)      .. "=menu  " ..
    getKeyName(keybinds.breakers)  .. "=breakers  " ..
